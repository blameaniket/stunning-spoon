...
static const char *rect_fs_source =
    "#version 330 core\n"
    "out vec4 FragColor;\n"
    "uniform vec4 color;\n"
    "void main()\n"
...
static void get_ortho_projection(float left, float right, float bottom, float top, float near_val, float far_val, float *mat) {
    for (int i = 0; i < 16; i++) {
        mat[i] = 0.0f;
    }
...
void renderer_init(void) {
    ...
    g_renderer_ctx.rect_proj_loc = glGetUniformLocation(g_renderer_ctx.rect_shader, "projection");
    g_renderer_ctx.rect_bounds_loc = glGetUniformLocation(g_renderer_ctx.rect_shader, "rect");
    g_renderer_ctx.rect_color_loc = glGetUniformLocation(g_renderer_ctx.rect_shader, "color");
...
void draw_rectangle(RendererRectangle rectangle) {
    ...
    glUniform4f(g_renderer_ctx.rect_bounds_loc, rectangle.x, rectangle.y, rectangle.width, rectangle.height);
    glUniform4f(g_renderer_ctx.rect_color_loc, rectangle.color.r, rectangle.color.g, rectangle.color.b, rectangle.color.alpha);
...
Font load_font(const char *file_path, unsigned int font_size) {
    ...
    if (!g_renderer_ctx.ft_initialized) {
        if (FT_Init_FreeType(&g_renderer_ctx.ft_lib)) {
            log_error("FreeType not initialized and failed to init on font load\n");
            return font;
        }
        g_renderer_ctx.ft_initialized = true;
    }
...
void draw_text(Font font, const char *text, Vector2 position, float scale, Color text_color) {
    ...
    glUniform4f(g_renderer_ctx.text_color_loc, text_color.r, text_color.g, text_color.b, text_color.alpha);
    ...
    for (const char *p = text; *p; p++) {
        ...
        if (c == '\n') {
            x = position.x;
            y += (font.line_height > 0 ? font.line_height : (int)font.size) * scale;
            continue;
        }
