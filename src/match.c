...
static inline bool is_word_boundary(const char *name, size_t i) {
    ...
    if (islower((unsigned char)prev) && isupper((unsigned char)curr)) return true;
    if (!isalnum((unsigned char)prev) && isalnum((unsigned char)curr)) return true;
    return false;
}
    ...
    for (; *haystack; haystack++) {
        if (tolower((unsigned char)*haystack) == tolower((unsigned char)*needle)) {
            ...
            for (k = 0; k < needle_len; k++) {
                if (!haystack[k] || tolower((unsigned char)haystack[k]) != tolower((unsigned char)needle[k]))
                    break;
            }
...
static int score_item(const char *name, const char *query) {
    ...
    for (size_t i = 0; i < name_len; i++) {
        ...
        for (size_t j = 0; j < query_len; j++) {
            ...
            bool match = (tolower(nc) == tolower(qc)) || (qc == ' ' && is_separator(nc));
            ...
            if (match) {
                ...
                if (j == 0) {
                    ...
                    if (D_prev[j - 1] != SCORE_MIN) {
                        score_consecutive = D_prev[j - 1] + 100 + 50 + case_bonus;
                    }

                    int score_gap = SCORE_MIN;
                    if (S_prev[j - 1] != SCORE_MIN) {
                        // Inner gap start penalty: -5
                        score_gap = S_prev[j - 1] + 100 + bonus + case_bonus - 5;
                    }

                    D_curr[j] = (score_consecutive > score_gap) ? score_consecutive : score_gap;
                } else {
                    D_curr[j] = SCORE_MIN;
                }
            } else {
            ...
            if (i == 0) {
                ...
                S_curr[j] = (D_curr[j] > prev_penalized) ? D_curr[j] : prev_penalized;
            }
        }
    ...
    if (sub != NULL) {
        if (sub == name) {
            ...
            // Contiguous substring at word boundary (e.g. "term" in "suckless-terminal")
            best_score += 3000;
        } else {
...
void sort_items(ConfigEntry *items, int total_items, const char *query) {
    if (!items || total_items <= 1 || !query || *query == '\0')
        return;
