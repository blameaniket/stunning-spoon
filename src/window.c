...
GLFWwindow *get_current_window() { return g_win_ctx.handle; }

int get_window_height() { return g_win_ctx.window_height; }


int get_window_width() { return g_win_ctx.window_width; }
...
static void char_callback(GLFWwindow *window, unsigned int codepoint) {
	...
	if (g_win_ctx.char_pressed_queue_count < CHAR_PRESSED_QUEUE_SIZE) {
		g_win_ctx.char_pressed_queue[g_win_ctx.char_pressed_queue_count++] =
		    (int)codepoint;
	}
}
