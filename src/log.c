...
static struct timespec get_realtime(void)
{
    ...
    const uint64_t WINDOWS_TO_UNIX_EPOCH =
        116444736000000000ULL;

    uint64_t unix_time =
        value.QuadPart - WINDOWS_TO_UNIX_EPOCH;

    struct timespec result;

    result.tv_sec =
        (time_t)(unix_time / 10000000ULL);

    result.tv_nsec =
        (long)((unix_time % 10000000ULL) * 100);
...
static struct timespec get_cpu_time(void)
{
    ...
    result.tv_sec =
        (time_t)(total_100ns / 10000000ULL);

    result.tv_nsec =
        (long)((total_100ns % 10000000ULL) * 100);
...
void log_debug(const char *const fmt, ...)
{
    ...
    if (GetProcessMemoryInfo(
        ...
        max_rss_kb =
            (long)(pmc.PeakWorkingSetSize / 1024);
    }
    ...
    fprintf(
        ...
        (long)real_time.tv_sec,
        (long)(real_time.tv_nsec / 1000),
        (long)cpu_time.tv_sec,
        (long)(cpu_time.tv_nsec / 1000),
        max_rss_kb
    );
...
)
{
    ...
    diff.tv_sec =
        cur.tv_sec - old.tv_sec;

    if (cur.tv_nsec >= old.tv_nsec) {

        diff.tv_nsec =
            cur.tv_nsec - old.tv_nsec;
