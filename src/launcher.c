...
void launcher_init(Launcher *launcher) {
	...
	launcher->result.items =
	    malloc(sizeof(ConfigEntry *) * launcher->items_count);
	...
	launcher->single_letter_width =
	    measure_text_length(launcher->window.font, "Z", 1.0);
}
...
void launcher_update(Launcher *launcher) {
	...
	if (is_key_pressed(KEY_BACKSPACE)) {
		if (launcher->cursor_pos > 0) {
			memmove(&launcher->query[launcher->cursor_pos - 1],
	...
	match_items(launcher->items, launcher->items_count, launcher->query,
	            launcher->result.items, &launcher->result.items_count);



	if (is_key_pressed(KEY_DOWN) || (ctrl && is_key_pressed(KEY_N)))
		launcher->selected_index++;
	if (is_key_pressed(KEY_UP) || (ctrl && is_key_pressed(KEY_P)))
		launcher->selected_index--;
	...
	if (is_key_pressed(KEY_ENTER)) {
		if (launcher->result.items_count > 0 && launcher->selected_index >= 0 &&
	...
	draw_text(launcher->window.font, launcher->window.prompt,
	          (Vector2){prompt_x, prompt_y}, 1.0f,
	          launcher->window.foreground_color);


	float spacing = 20.0f;
	float prompt_width = measure_text_length(launcher->window.font,
	                                         launcher->window.prompt, 1.0f);

	// draw query
	float query_x = prompt_x + prompt_width + spacing;
	draw_text(launcher->window.font, launcher->query,
	          (Vector2){query_x, prompt_y}, 1.0f,
	          launcher->window.foreground_color);

	// cursor position
	float query_width =
	    measure_text_length(launcher->window.font, launcher->query, 1.0f);
	// float calculate_cursor_pos = launcher->cursor_pos == 0 ? 0 :
	// launcher->cursor_pos + 1;
	float calculate_cursor_pos = launcher->cursor_pos;
	float cursor_x =
	    query_x + launcher->single_letter_width * calculate_cursor_pos;

	draw_rectangle(
	    (RendererRectangle){.x = cursor_x,
		                    .y = prompt_y,
	...
	float line_height = launcher->window.font.line_height > 0
	                        ? (float)launcher->window.font.line_height
	                        : 45.0f;
	...
	for (int i = 0; i < launcher->result.items_count; i++) {
		...
		Color font_color = (launcher->selected_index == i)
		                       ? launcher->window.selected_item_color
		                       : launcher->window.foreground_color;

		draw_text(launcher->window.font, launcher->result.items[i]->name,
		          (Vector2){margin_x, margin_y}, 1.0f, font_color);
